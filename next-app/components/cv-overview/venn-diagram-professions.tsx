...
import styles from "./venn-diagram-professions.module.scss";
  ...
  return (
	...
	l-1.3,3.6c-9.3,22.1-21.9,35.9-35.8,45.2l-2.3,8.1l-2.3,7c-17.4,53.4-40.7,87.1-74.3,105.7h-1.2C233.1,484.4,213.4,489,194.8,489z"
      ...
      <path
        ...
        d="M215.7,42.9c-7,1.2-8.1,9.3-3.5,12.8c4.6,3.5,3.5,10.5-2.3,12.8h-30.2c-31.4,0-33.7,0-44.1,2.3
...
export function VennDiagramProfessions() {
  const [basicFontSize, setBasicFontSize] = useState<number | null>(null);
  ...
  const fontSizeStyle = basicFontSize ? { fontSize: `${basicFontSize}px` } : {};

  return (
    <div className={styles.vennDiagramProfessions} style={fontSizeStyle}>
      <div ref={wrapperRef} className={styles.wrapper}>
        <div className={styles.circleLeft}>
          <span className={styles.circleTitle}>Back End</span>
        </div>
        <div className={styles.circleTop}>
          <span className={styles.circleTitle} style={{ fontSize: "1.45em" }}>
            Front End
          </span>
        </div>
        <div className={styles.circleRight}>
          <span className={styles.circleTitle}>UI/UX Design</span>
        </div>
        <div className={styles.centerArea}>
          <span className={styles.circleTitle}>
            Unicorn
            <br />
            Area
          </span>
        </div>
        <div className={styles.meArea}>
          <span className={styles.pin}>
            <HeadIcon />
          </span>
          <span className={styles.speechBubble}>Hi! It&apos;s me!</span>
        </div>
      </div>
    </div>
  );
